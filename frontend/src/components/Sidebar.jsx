...
export default function Sidebar({ isOpen, onToggle, isMobile, onClose }) {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <aside
        data-testid="sidebar"
        className={`fixed left-0 top-0 h-screen z-50 glass-strong transition-all duration-300 flex flex-col
          ${isMobile ? 'w-[280px]' : (isOpen ? 'w-[260px]' : 'w-[72px]')}`}
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {(isOpen || isMobile) && <span className="text-lg font-light tracking-tight text-white/90" style={{ fontFamily: 'Outfit' }}>Keeps</span>}
          </div>
          {isMobile && (
            <button data-testid="close-sidebar" onClick={onClose} className="text-zinc-500 hover:text-zinc-300 p-1.5 hover:bg-white/5 rounded-lg transition-colors">
            ...
            const btn = (
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-xl transition-all duration-200 ${
                  showLabel ? 'px-4 py-3' : 'px-0 py-3 justify-center'
                } ${isActive
            ...
            if (!showLabel) {
              return (
                <Tooltip key={item.path}>
                  ...
                  <TooltipContent side="right" className="bg-zinc-800 text-zinc-200 border-white/10">{item.label}</TooltipContent>
                </Tooltip>
              );
            }
            return btn;
        ...
        <div className="px-3 mb-2">
          {(isOpen || isMobile) ? (
            <div className="glass-card rounded-xl p-4 glow-amber cursor-pointer" onClick={() => handleNav("/taxes")}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex justify-center py-2 cursor-pointer" onClick={() => handleNav("/taxes")}>
        ...
        <div className="px-3 pb-4 space-y-2">
          ...
          <div className="flex items-center gap-1">
            {!isMobile && (
              <button data-testid="sidebar-toggle" onClick={onToggle} className="flex-1 flex items-center justify-center py-2 text-zinc-500 hover:text-zinc-300 hover:bg-white/5 rounded-lg transition-colors">
            ...
            <Tooltip>
              ...
              {!isMobile && <TooltipContent side="right" className="bg-zinc-800 text-zinc-200 border-white/10">Sign Out</TooltipContent>}
            </Tooltip>
          </div>
        </div>
      </aside>
